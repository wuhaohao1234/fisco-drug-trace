<template>
  <div class="p-10 bg-white min-h-full rounded">
    ...
    <div class="flex-end">
      ...
      <el-button type="danger" @click="openAddForm" v-if="showAdd"
        >添加</el-button
      >
      <slot name="btns"></slot>
    </div>
        ...
        <el-table :data="tableDate" stripe :border="true" style="width: 100%">
          <template v-for="(itemProps, index) in tableColProps" :key="index">
            <el-table-column
              v-bind="itemProps"
              align="center"
              :width="itemProps.width ? itemProps.width : 'auto'"
            >
              <template #default="scope">
                <slot
                  :name="
                    itemProps.slotName ? itemProps.slotName : itemProps.prop
                  "
                  :scope="scope"
                  :row="scope.row"
                  :index="scope.$index"
                  ...
                  <template v-if="itemProps.type === 'time'">
                    {{
                      scope.row[itemProps.prop] != null
                        ? moment(scope.row[itemProps.prop]).format(
                            "YYYY-MM-DD HH:mm:ss"
                          )
                        : ""
                    }}
      ...
      <slot name="form">
        ...
        >
          <template v-for="(itemProps, index) in formProps" :key="index">
            <slot
              :name="itemProps.slotName ? itemProps.slotName : itemProps.prop"
              :index="index"
              :formData="formData"
            >
              <el-form-item :label="itemProps.label" :prop="itemProps.prop">
                ...
                <template v-if="itemProps.type === 'textarea'">
                  <el-input
                    v-model="formData[itemProps.prop]"
                    type="textarea"
                    autosize
                  ></el-input>
                </template>
                ...
                <template v-if="itemProps.type === 'select'">
                  ...
                  >
                    <el-option
                      v-for="(item, index) in itemProps.options"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                    >
                ...
                <template v-if="itemProps.type === 'switch'">
                  <el-switch
                    v-model="formData[itemProps.prop]"
                    inline-prompt
                    :active-text="itemProps.activeText"
                    :inactive-text="itemProps.inactiveText"
                    :active-value="itemProps.activeValue"
                    :inactive-value="itemProps.inactiveValue"
                  />
                </template>
...
const handleSuccess = (response, file, fileList) => {
  ...
  this.formData.image = response.url; // 假设后端返回的URL存放在response.url中
};
